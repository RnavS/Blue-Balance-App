...
// leaves no legal way to sell Premium on iOS. Rather than ship a paywall iOS
// users can never pass, iOS gets every Premium feature for free until StoreKit
// lands in 1.1.
//
...
// and no purchase path exists to undercut. Remove this module — and the
// `platform` argument its callers read — when IAP ships.
const FREE_PREMIUM_PLATFORMS = new Set(["ios"]);

export function hasPremiumAccess(isActive: boolean, platform: unknown): boolean {
  if (isActive) return true;

  // Kill switch: set IOS_PREMIUM_FREE=false to re-enable the paywall everywhere
  // without redeploying code.
  if (Deno.env.get("IOS_PREMIUM_FREE") === "false") return false;
