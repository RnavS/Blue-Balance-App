...
import { ScanBarcode, Camera, Check, Plus, Trash2, History as HistoryIcon, X, AlertCircle, Loader2 } from 'lucide-react';
...
export function QuickScan() {
  ...
  const startBarcodeScanning = useCallback(() => {
    ...
    scanIntervalRef.current = window.setInterval(async () => {
      ...
      try {
        ...
        // BarcodeDetector not available or failed, continue polling
      }
    }, 500);
  }, [stopCamera]);

  useEffect(() => {
    // Create canvas for barcode scanning
  ...
  const lookupBarcode = async (barcode: string) => {
    ...
    if (barcodeCache[barcode]) {
      setBeverageFromData(barcodeCache[barcode], barcode);
      setLoading(false);
      return;
    }
    
    // Check saved scanned beverages
    const savedBeverage = scannedBeverages.find(b => b.barcode === barcode);
    if (savedBeverage) {
      ...
      setBeverageFromData(data, barcode);
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (offData.status === 1 && offData.product) {
        ...
        // Estimate hydration factor based on category
    ...
    setBeverageFromData(defaultData, barcode);
    setLoading(false);
  ...
  const handleConfirm = async () => {
    ...
    if (!scannedBeverages.find(b => b.barcode === scannedBarcode)) {
    ...
    >
      ...
      <div className="space-y-4">
        ...
        >
          ...
          <p className="text-xs text-muted-foreground">Enter barcode number or use camera above</p>
